...
10. **Portable constraints are declarative.** Arbitrary Python validation logic is not a portable contract.
11. **Escape hatches remain available.** Advanced users can access PyArrow types, namespaced custom constraints, and raw metadata.
12. **ArrowBound defines; it does not execute.** Dataframes, databases, query engines, registries, and ETL systems remain outside the package.
13. **Portability over convenience hacks.** Never make schema construction easier by making meaning ambiguous.
14. **No unexplained gaps.** Every relevant PyArrow type and applicable Pydantic constraint should have a known support classification.
...
ArrowBound is deliberately not a dataframe library, ETL framework, ORM, query engine, schema registry, database migration system, Arrow table validation engine, engine integration layer, or arbitrary-Pydantic-to-Arrow converter. ArrowBound defines contracts; other systems consume them through Apache Arrow.
...
- `constraints.py`: Pydantic constraint extraction and normalization.
